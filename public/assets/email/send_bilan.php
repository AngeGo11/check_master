...
require_once $_SERVER['DOCUMENT_ROOT'] . '/GSCV/PHPMailer/src/Exception.php';
require_once $_SERVER['DOCUMENT_ROOT'] . '/GSCV/PHPMailer/src/PHPMailer.php';
require_once $_SERVER['DOCUMENT_ROOT'] . '/GSCV/PHPMailer/src/SMTP.php';
...
try {
    ...
    // Vérifier si le fichier existe avec le chemin original
    if (!file_exists($filePath)) {
        // Essayer avec le chemin relatif
        $relativePath = '../../' . $compteRendu['fichier_cr'];
        if (file_exists($relativePath)) {
            $filePath = $relativePath;
        } else {
            throw new Exception('Fichier du compte rendu introuvable. Chemin essayé: ' . $filePath . ' et ' . $relativePath);
        }
    }
