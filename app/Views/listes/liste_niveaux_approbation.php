...
if (!isset($_GET['liste']) || $_GET['liste'] !== 'niveaux_approbation') {
    return;
}
...
$params = [];
if ($search !== '') {
    $where = 'WHERE lib_niveau_approbation LIKE ?';
    $params[] = "%$search%";
}

// Compter le total pour la pagination
$sql_count = "SELECT COUNT(*) FROM niveau_approbation $where";
...
$sql = "SELECT * FROM niveau_approbation $where ORDER BY id_approb LIMIT $per_page OFFSET $offset";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['delete_selected_ids']) && is_array($_POST['delete_selected_ids'])) {
        ...
        header('Location: ?page=parametres_generaux&liste=niveaux_approbation');
        exit;
    }
