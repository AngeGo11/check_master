...
<body>
    <div class="header">
        <div class="header-title">
            ...
            <div class="text-container">
                <h1>Liste des Niveaux d'Études</h1>
                <p>Gestion des niveaux d'études du système</p>
            </div>
        </div>
    ...
    <div class="actions-bar">
        ...
        <form method="GET" class="search-box" style="display:inline-flex;align-items:center;gap:5px;">
            ...
            <input type="text" name="search" placeholder="Rechercher un niveau d'études..." value="<?php echo htmlspecialchars($search); ?>">
            <button type="submit" class="button" style="margin-left:5px;">Rechercher</button>
        </form>
        <button class="button" onclick="showAddModal()">
            <i class="fas fa-plus"></i> Ajouter un niveau d'études
        </button>
    </div>
    ...
    <div class="data-table-container">
        <div class="data-table-header">
            <div class="data-table-title">Liste des niveaux d'études (<?php echo $total_niveaux; ?> éléments)</div>
        </div>
        <table class="data-table">
            ...
            <tbody>
                <?php if (count($niveaux) === 0): ?>
                    <tr>
                        <td colspan="4" style="text-align:center;">Aucun niveau d'études trouvé.</td>
                    </tr>
                <?php else: ?>
                    <?php foreach ($niveaux as $niveau): ?>
                        <tr>
                            <td><input type="checkbox" class="row-checkbox" value="<?php echo htmlspecialchars($niveau['id_niv_etd']); ?>"></td>
                            <td><?php echo htmlspecialchars($niveau['id_niv_etd']); ?></td>
                            <td><?php echo htmlspecialchars($niveau['lib_niv_etd']); ?></td>
                            <td>
                                <div class="action-buttons">
                                    <button class="action-button edit-button" title="Modifier" onclick="editNiveau(<?= $niveau['id_niv_etd']; ?>, '<?= htmlspecialchars(addslashes($niveau['lib_niv_etd'])); ?>')">
                                        <i class="fas fa-pen"></i>
                                    </button>
                                    <button class="action-button delete-button" title="Supprimer" onclick="showDeleteModal(<?= $niveau['id_niv_etd']; ?>, '<?= htmlspecialchars(addslashes($niveau['lib_niv_etd'])); ?>')">
    ...
    <div class="pagination">
        <?php if ($page > 1): ?>
            <a class="page-item" href="?page=parametres_generaux&liste=niveaux_etudes&search=<?php echo urlencode($search); ?>&page=1">«</a>
            <a class="page-item" href="?page=parametres_generaux&liste=niveaux_etudes&search=<?php echo urlencode($search); ?>&page=<?php echo $page - 1; ?>">‹</a>
        <?php endif; ?>
        <?php
        ...
        for ($i = $start; $i <= $end; $i++):
        ?>
            <a class="page-item<?php if ($i == $page) echo ' active'; ?>" href="?page=parametres_generaux&liste=niveaux_etudes&search=<?php echo urlencode($search); ?>&page=<?php echo $i; ?>"><?php echo $i; ?></a>
        <?php endfor; ?>
        <?php if ($page < $total_pages): ?>
            <a class="page-item" href="?page=parametres_generaux&liste=niveaux_etudes&search=<?php echo urlencode($search); ?>&page=<?php echo $page + 1; ?>">›</a>
            <a class="page-item" href="?page=parametres_generaux&liste=niveaux_etudes&search=<?php echo urlencode($search); ?>&page=<?php echo $total_pages; ?>">»</a>
        <?php endif; ?>
    </div>

    <!-- Modal pour ajouter/modifier un niveau d'études -->
    <div id="niveauModal" class="modal">
        <div class="modal-content">
            <div class="modal-header">
                <h2 id="modalTitle">Ajouter un niveau d'études</h2>
                <span class="close">&times;</span>
            </div>
            <form id="niveauForm" method="POST">
                <input type="hidden" name="page" value="parametres_generaux">
                <input type="hidden" name="liste" value="niveaux_etudes">
                <input type="hidden" id="niveauId" name="id_niv_etd">
                <div class="form-group">
                    <label for="lib_niv_etd">Libellé :</label>
                    <input type="text" id="lib_niv_etd" name="lib_niv_etd" required>
                </div>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="deleteForm" method="POST">
                    ...
                    <p id="deleteMessage">Êtes-vous sûr de vouloir supprimer ce niveau d'études ?</p>
                    <div class="modal-footer">
    ...
    <script>
        ...
        function editNiveau(id, libelle) {
            document.getElementById('modalTitle').textContent = 'Modifier un niveau d\'études';
        ...
        function showDeleteModal(id, libelle) {
            ...
            document.getElementById('deleteMessage').textContent = "Êtes-vous sûr de vouloir supprimer le niveau d'études : '" + libelle + "' ?";
        ...
        function openDeleteMultipleModal() {
            ...
            if (checked.length === 0) {
                msg.innerHTML = "Aucune sélection. Veuillez sélectionner au moins un niveau d'études à supprimer.";
                footer.innerHTML = '<button type="button" class="button" onclick="closeDeleteMultipleModal()">OK</button>';
            } else {
                msg.innerHTML = `Êtes-vous sûr de vouloir supprimer <b>${checked.length}</b> niveau(x) d'études sélectionné(s) ?<br><span style='color:#c0392b;font-size:0.95em;'>Cette action est irréversible.</span>`;
