<script>
    ...
    const ONLYOFFICE_CONFIG = {
        documentServer: 'http://localhost:80', // URL de votre serveur OnlyOffice
        apiUrl: 'http://localhost:80/web-apps/apps/api/documents/api.js',
        token: '<?php echo isset($_SESSION['onlyoffice_token']) ? $_SESSION['onlyoffice_token'] : ''; ?>'
    };
    ...
    document.addEventListener('DOMContentLoaded', function() {
        // Récupérer le token OnlyOffice
        fetchOnlyOfficeToken();
        ...
        if (newReportBtn && forbiddenModal) {
            newReportBtn.addEventListener('click', function(e) {
                if (hasExistingReport) {
                    ...
                    // Ouvre directement la modale de création
                    e.preventDefault();
                    if (createReportModal) {
                        createReportModal.style.display = 'flex';
                        setTimeout(() => createReportModal.classList.add('open'), 10);
                        // Initialiser l'éditeur quand la modale s'ouvre
                        setTimeout(() => {
    ...
    // Récupérer le token OnlyOffice
    async function fetchOnlyOfficeToken() {
        try {
            const response = await fetch('/GSCV+/public/assets/traitements/generate_onlyoffice_token.php');
            const data = await response.json();
            
            if (data.success) {
                ONLYOFFICE_CONFIG.token = data.token;
                console.log('Token OnlyOffice récupéré avec succès');
            } else {
                console.warn('Impossible de récupérer le token OnlyOffice');
            }
        } catch (error) {
            console.error('Erreur lors de la récupération du token OnlyOffice:', error);
        }
    }

    // Initialiser la synchronisation de tous les champs
    function initializeFieldSync() {
        // Synchronisation du thème
        const themeInput = document.getElementById('theme_report');
        if (themeInput) {
            themeInput.addEventListener('input', function() {
                currentTheme = this.value.trim();
                updateAllFieldsInEditor();
            });
        }

        // Synchronisation du nom de l'étudiant
        const nomInput = document.getElementById('nom_etudiant');
        if (nomInput) {
        ...
        // Synchronisation de l'encadreur
        const encadreurInput = document.getElementById('encadreur');
        if (encadreurInput) {
            encadreurInput.addEventListener('input', function() {
                updateAllFieldsInEditor();
            });
        }
    }

    // Mettre à jour tous les champs dans l'éditeur
    function updateAllFieldsInEditor() {
        if (!window.localEditor) return;

        const editor = window.localEditor;
        const content = editor.innerHTML;
        
        // Récupérer toutes les valeurs des champs
        const theme = document.getElementById('theme_report')?.value.trim() || '';
        const nom = document.getElementById('nom_etudiant')?.value.trim() || '';
        const encadreur = document.getElementById('encadreur')?.value.trim() || '';
        ...
        if (theme) {
            updatedContent = updatedContent.replace(
                /<h2[^>]*>Thème du mémoire<\/h2>/g,
                `<h2>Thème du mémoire</h2>`
            ).replace(
                /<p[^>]*>Veuillez remplacer ce texte par le thème de votre mémoire\.<\/p>/g,
                `<p><strong>${theme}</strong></p>`
            ).replace(
                /MISE EN PLACE D'UN MODULE D'INTÉGRATION ENTRE ATLANTIS CRM ET ATLANTIS SGO :<br>CAS DE LA BOA CAPITAL ASSET MANAGEMENT/g,
                theme
            );
        }

        // Remplacer le nom de l'étudiant
        if (nom) {
            updatedContent = updatedContent.replace(
                /<p[^>]*>Nom et prénom de l'étudiant<\/p>/g,
                `<p><strong>${nom}</strong></p>`
            ).replace(
                /Nom et prénom de l'étudiant/g,
                nom
            );
        }

        // Remplacer l'encadreur
        if (encadreur) {
            updatedContent = updatedContent.replace(
                /<p[^>]*>Nom de l'encadreur<\/p>/g,
                `<p><strong>${encadreur}</strong></p>`
            ).replace(
                /Nom de l'encadreur/g,
                encadreur
            );
        }
        ...
        if (content !== updatedContent) {
            ...
            console.log('Champs mis à jour dans l\'éditeur');
        }
    }
    ...
    function initializeOnlyOfficeEditor() {
        ...
        console.log('Initialisation de l\'éditeur OnlyOffice...');
        ...
        if (window.localEditor) {
            console.log('Éditeur local initialisé avec succès');
        ...
        // Initialiser OnlyOffice si le token est disponible
        if (ONLYOFFICE_CONFIG.token) {
            initializeOnlyOfficeWithToken();
        } else {
            console.warn('Token OnlyOffice non disponible, utilisation de l\'éditeur local');
        }
    }

    // Initialiser OnlyOffice avec token
    function initializeOnlyOfficeWithToken() {
        // Charger l'API OnlyOffice
        const script = document.createElement('script');
        script.src = ONLYOFFICE_CONFIG.apiUrl;
        script.onload = function() {
            console.log('API OnlyOffice chargée');
            createOnlyOfficeEditor();
        };
        script.onerror = function() {
            console.error('Erreur lors du chargement de l\'API OnlyOffice');
        };
        document.head.appendChild(script);
    }

    // Créer l'éditeur OnlyOffice
    function createOnlyOfficeEditor() {
        const onlyofficeContainer = document.getElementById('onlyoffice-container');
        if (!onlyofficeContainer) return;

        // Générer une clé unique pour le document
        documentKey = 'rapport_' + Date.now() + '_' + Math.random().toString(36).substr(2, 9);
        ...
        const config = {
            document: {
                ...
                url: ONLYOFFICE_CONFIG.documentServer + '/storage/templates/modele_rapport_de_stage.docx'
            },
            documentType: 'word',
            editorConfig: {
                mode: 'edit',
                callbackUrl: window.location.origin + '/GSCV+/public/assets/traitements/onlyoffice_callback.php',
                user: {
                    id: '<?php echo $_SESSION['user_id'] ?? 'user'; ?>',
                    name: '<?php echo $_SESSION['nom'] ?? 'Utilisateur'; ?>'
                },
                customization: {
                    chat: false,
                    comments: true,
        ...
        // Créer l'éditeur OnlyOffice
        onlyOfficeEditor = new DocsAPI.DocEditor(onlyofficeContainer, config);
        
        console.log('Éditeur OnlyOffice créé avec succès');
    }

    // Charger un modèle dans OnlyOffice
    async function loadModelInOnlyOffice(selectedModel) {
        if (!onlyOfficeEditor) {
            throw new Error('Éditeur OnlyOffice non initialisé');
        }

        console.log('Chargement du modèle dans OnlyOffice:', selectedModel);

        // Mettre à jour la configuration du document
        const newConfig = {
            document: {
                fileType: selectedModel.endsWith('.docx') ? 'docx' : 'html',
                key: documentKey,
                title: 'Rapport de Stage',
                url: ONLYOFFICE_CONFIG.documentServer + '/storage/templates/' + selectedModel
            },
            documentType: 'word',
            editorConfig: {
                mode: 'edit',
        ...
        // Recharger l'éditeur avec le nouveau modèle
        const onlyofficeContainer = document.getElementById('onlyoffice-container');
        onlyofficeContainer.innerHTML = '';
        onlyOfficeEditor = new DocsAPI.DocEditor(onlyofficeContainer, newConfig);

        console.log('Modèle chargé dans OnlyOffice');
    }

    // Charger un modèle dans l'éditeur local
    async function loadModelInLocalEditor(selectedModel) {
        ...
        } else if (selectedModel.endsWith('.html')) {
        ...
        console.log('Modèle chargé dans l\'éditeur local');
    }
        ...
        if (loadTemplateBtn) {
            ...
            loadTemplateBtn.addEventListener('click', async function() {
                ...
                try {
                    ...
                    // Utiliser OnlyOffice si disponible, sinon l'éditeur local
                    if (onlyOfficeEditor && ONLYOFFICE_CONFIG.token) {
                        await loadModelInOnlyOffice(selectedModel);
                    } else {
                        await loadModelInLocalEditor(selectedModel);
                    }
